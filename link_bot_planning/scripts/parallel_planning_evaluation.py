...
def main(args):
    gz_processes = []
    planning_processes = []
    num_parallel_data_collection_threads = 3
    ...
    for process_idx in range(num_parallel_data_collection_threads):
        ...
        planning_cmd = ["python", "scripts/planning_evaluation2.py", args.planner_params, "test_scenes/car4_alt", args.outdir, args.dynamics, args.mde, f"--trials={trials_set}", "--on-exception=raise"]
        port_num += 2
...
if __name__ == "__main__":
    ...
    parser.add_argument("--outdir", type=pathlib.Path, default="/media/shared/planning_results/gazebo_parallel6", help='used in making the output directory')
