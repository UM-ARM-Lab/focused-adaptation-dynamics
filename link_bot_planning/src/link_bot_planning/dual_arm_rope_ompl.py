...
class DualArmRopeOmpl(FloatingRopeOmpl):
    ...
    def ompl_state_to_numpy(self, ompl_state: ob.CompoundState):
        ...
        error = np.array([ompl_state[state_component_idx][0]], np.float32)
        state_component_idx += 1
    ...
    def make_state_space(self):
        ...
        error_subspace = ob.RealVectorStateSpace(1)
        error_bounds = ob.RealVectorBounds(1)
        error_bounds.setLow(-1000)
        error_bounds.setHigh(1000)
        error_subspace.setBounds(error_bounds)
        error_subspace.setName("error")
        state_space.addSubspace(error_subspace, weight=0)
    ...
    def make_goal_region(self,
        ...
        if goal['goal_type'] == 'midpoint':
            ...
            return DualGrippersGoalRegion(si=si,
                                          scenario_ompl=self,
                                          rng=rng,
                                          threshold=params['goal_params']['threshold'],
                                          goal=goal,
...
# noinspection PyMethodOverriding
class DualGrippersGoalRegion(floating_rope_ompl.GripperGoalRegion):
