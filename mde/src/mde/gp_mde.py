...
class GPRMDE(MDE):
    ...
    def make_gp(self):
        ...
        self.model_homoskedastic = DeepExactGP(self._train_batch, error_labels, self.likelihood, self.input_data_to_gp_input)
        self.model_homoskedastic.mean_module = gpytorch.means.ConstantMean().cuda()
        self.model_homoskedastic.covar_module = gpytorch.kernels.ScaleKernel(gpytorch.kernels.RBFKernel(ard_num_dims=self._gp_input_dim)).cuda()
        self.mll = gpytorch.mlls.ExactMarginalLogLikelihood(self.model_homoskedastic.likelihood, self.model_homoskedastic)
        self.model = self.model_homoskedastic

    def make_heteroskedastic_gp(self):
        error_labels = self.error_scaler.transform(self._train_error_labels).cuda()
        self.model_homoskedastic.eval()
        observed_var = torch.pow(self.model_homoskedastic(self.model_homoskedastic.train_inputs).mean - self.model_homoskedastic.train_targets, 2)
    ...
    def _get_unscaled_mean_and_std(self, dhat_batch):
        ...
        std_unscaled = (std_scaled * self.error_scaler.std).cpu().detach().numpy().flatten()
    ...
    def training_epoch_end(self, inp):
        ...
        self.log_dgt_dhat_table_and_plot(dhat_batch.mean.cpu().detach().numpy().flatten(), self._error_forward_tf(self._train_error_labels).cpu().detach().numpy().flatten(), std_scaled.cpu().detach().numpy().flatten(), label="train_scaled_space")
        self.log_dgt_dhat_table_and_plot(train_d_hats.flatten(), self._train_error_labels.cpu().detach().numpy().flatten(),std_unscaled, label="train")
        self.train_d_hats = []
    ...
    def validation_step(self, val_batch: Dict[str, torch.Tensor], batch_idx):
        ...
        pred_error = self._eval_model_on_data(val_batch)
        std_scaled = torch.sqrt(pred_error.variance)
        #print("Val variance", std)
        #print("Val mean", pred_error.mean)
        try:
        ...
        #self.val_n
        #self.val_tp = torchmetrics.SumMetric()
        #self.val_fp = torchmetrics.SumMetric()
        self.log('pred_minus_true_error', signed_loss)
        #Bookkeeping
        ...
        self.val_stds.extend(std.detach().cpu().numpy().flatten())
        return loss
