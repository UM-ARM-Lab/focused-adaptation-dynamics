...
def analyze_planning_results(args):
    ...
    _, ax = boxplot(df, outdir, hue, 'normalized_model_error', "Model Error", figsize=(12, 8), palette=palette, save=False)
    ax.set_xlabel(None)
    plt.savefig(outdir / 'real_model_error.png', dpi=200)

    ax = barplot_with_values(df, 'any_solved', hue, outdir, figsize=(12, 8), title="Plan to Goal Found",
                             palette=palette)
    ax.set_xlabel(None)
    plt.savefig(outdir / 'real_plan_to_goal_found.png', dpi=200)

    ax = barplot_with_values(df, 'success', hue, outdir, figsize=(12, 8), palette=palette,
